...
We follow the \link{https://documentation.its.umich.edu/node/5013}{greatlakes user guide for getting started with the command line}. As preliminaries, you need:
\begin{itemize}
  \item A Slurm account. You should already have a primary account, \code{stats\_dept1}, and a smaller backup account for if you exhaust your resourses, \code{stats\_dept2}. 
   \item A greatlakes cluster login account. If you have not yet filled in the form at \url{https://arc.umich.edu/login-request} then do so.
...
\begin{frame}{Connecting to greatlakes with a browser}
\begin{enumerate}
\item Point your browser to \url{https://greatlakes.arc-ts.umich.edu}
\item Choose the menu option: Clusters $\rightarrow$ Great Lakes Shell Access
\end{enumerate}
...
\\
\code{my\_accounts} & show all billing accounts on which you can run jobs
\end{tabular}
...
module avail R
@
We see that R4.4.0 is currently the default. For simple multicore computing, the default R module is appropriate. Other versions of R have been built and tested in other parallel environments, for example the \texttt{Rmpi} module runs R with mpi.
...
\begin{itemize}
\item
The \texttt{gl} subdirectory of the \texttt{810f24} git repository has a file \texttt{test.sbat} which submits a batch job running the parallel \texttt{foreach} test in \texttt{test.R}.
\item You can transfer the files from your laptop via \texttt{scp}, or by copy-paste.
Or simply clone the class git repository into your greatlakes account,
<<f12,echo=T,eval=F>>=
git clone https://github.com/ionides/810f24.git
@
\item Inspect the text file \texttt{test.sbat}, for example by
<<f13,echo=T,eval=F>>=
more test.sbat
@
\item One thing that needs changing is to set your email address for alerts about jobs beginning and ending. To make these edits on greatlakes, you need a text editor. Options include
<<f14,echo=T,eval=F>>=
vi test.sbat
emacs -nw test.sbat
nano test.sbat
...
\item You can also run \link{https://greatlakes.arc-ts.umich.edu/pun/sys/dashboard/batch_connect/sys/arcts_rstudio/session_contexts/new}{web-based Rstudio}.
...
\begin{frame}{Acknowledgments}
  \begin{itemize}
  \item
    This lesson is prepared for STATS 810, Fall 2024.
  \item
